...
//		logger.debug("decode queryStr ={}, method={}, msg={}", queryStr, method, request);
		...
		out.add(content);
		boolean alive = false;// test
//				HttpHeaderUtil.isKeepAlive(request);
...
//				HttpHeaderUtil.isKeepAlive(request);
	...
	protected void sendHttpResponse(ChannelHandlerContext ctx, FullHttpRequest req, FullHttpResponse res) {
		...
		if (res.status().code() != 200) {
			...
			buf.release();
//			HttpHeaderUtil.setContentLength(res, res.content().readableBytes());//test
		}

		// Send the response and close the connection if necessary.
		ChannelFuture f = ctx.channel().writeAndFlush(res);
//		if (!HttpHeaderUtil.isKeepAlive(req) || res.status().code() != 200) {//test
//			f.addListener(ChannelFutureListener.CLOSE);//test
//		}
	}
