...
import com.lioncorp.nettythrift.core.ThriftMessageWrapper;
import com.lioncorp.nettythrift.core.ThriftServerDef;
import com.lioncorp.nettythrift.core.ThriftServerDefBuilderBase;
import com.lioncorp.nettythrift.protocol.ProtocolFactorySelector;
...
public class ThriftMessageDecoder extends ByteToMessageDecoder {
	...
	public ThriftMessageDecoder(ThriftServerDef serverDef, ProtocolFactorySelector protocolFactorySelector) {
		this.serverDef = serverDef;
		this.protocolFactorySelector = (null == serverDef)?
				protocolFactorySelector : serverDef.protocolFactorySelector;		
		this.maxFrameSize = (null == serverDef)? 
			ThriftServerDefBuilderBase.MAX_FRAME_SIZE : serverDef.maxFrameSize;
	...
	protected ThriftMessage decodeMessage(ChannelHandlerContext ctx, ByteBuf buffer)
		...
		if (inputProtocolFactory != null) {
			...
			return new ThriftMessage(messageBuffer, inputProtocolFactory, ThriftTransportType.UNFRAMED).setWrapper(unframedMessageWrapper(successor));
			...
			return new ThriftMessage(messageBuffer, inputProtocolFactory, ThriftTransportType.FRAMED).setWrapper(framedMessageWrapper(successor));
		}
	}
	...
	private TProtocolFactory getProtocolFactory(ByteBuf buffer) {
		...
		if (headCode != 0 && buffer.readableBytes() > MESSAGE_FRAME_SIZE) {
			...
			return protocolFactorySelector.getProtocolFactory(headCode);
		}
		return null;
	}
