...
import com.lioncorp.nettythrift.processor.NettyProcessorFactory;
import com.lioncorp.nettythrift.protocol.DefaultProtocolFactorySelectorFactory;
import com.lioncorp.nettythrift.protocol.ProtocolFactorySelectorFactory;
...
public abstract class ThriftServerDefBuilderBase<T extends ThriftServerDefBuilderBase<T>> {
	...
	private TBaseProcessor baseProcessor;
	private Map<String, TBaseProcessor<?>> baseProcessors;
	...
	@SuppressWarnings("unchecked")
	public T withProcessor(@SuppressWarnings("rawtypes") TBaseProcessor baseProcessor) {
	...
	@SuppressWarnings("unchecked")
	public T withProcessors(Map<String, TBaseProcessor<?>> baseProcessors) {
	...
	public ThriftServerDef build() {
		checkState(baseProcessor != null, "Processor not defined!");
		...
		return new ThriftServerDef(name, 
				"",
				serverPort, maxFrameSize, maxConnections, queuedResponseLimit,
	...
	public ThriftServerDef[] builds() {
		checkState(MapUtils.isNotEmpty(baseProcessors), "Processor not defined!");
...
//					  new ArrayBlockingQueue<Runnable>(100), 
					  new LimitedQueue<Runnable>(50000),
		...
		List<ThriftServerDef> list = null;
		if(null != baseProcessors && !baseProcessors.isEmpty()){
			list = Lists.newArrayList();
			for(Map.Entry<String, TBaseProcessor<?>> tmp : baseProcessors.entrySet()){
				list.add(new ThriftServerDef(
						name, 
						tmp.getKey(),
						serverPort, 
		...
		if(null != list && !list.isEmpty()){
			return list.toArray(new ThriftServerDef[0]);
		}
		return new ThriftServerDef[0];
	}
