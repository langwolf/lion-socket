...
public class TNettyThriftServer extends TServer {
    ...
    private static String bossGroupSize = System.getProperty("netty.server.bossgroup.size");
    private static String workerGroupSize = System.getProperty("netty.server.workergroup.size");
    private static int bossSize;
    private static int workSize;
    static {
    	bossSize = StringUtils.isNotBlank(bossGroupSize)? Integer.valueOf(bossGroupSize) : 0;
    	workSize = StringUtils.isNotBlank(workerGroupSize)? Integer.valueOf(workerGroupSize) : 0;	
    }  
    public static class Args extends AbstractServerArgs<Args> {
        ...
        public Args(int port, long timeout) {         	      	
            super(new TNettyThriftServerTransport(port, timeout, bossSize, workSize));
            this.clientTimeout = timeout;
            logger.info("netty.server.bossgroup.size:{}", bossGroupSize);
        }
